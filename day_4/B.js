...
let cardsInPlay = bingoCards;
let finalScore = -Infinity;
let totalCardCount = cardsInPlay.length;
let index = 0;
for(let i of drawOrder){
  	let str = `DRAW: ${index}`;
  	cardsInPlay.forEach(card=>markNumber(i, card));
  	
  	if(cardsInPlay.some(isWinningCard)){
      	
      	if(cardsInPlay.length == 1){
         	finalScore = calculateScore(cardsInPlay[0], i) 
			break;
        }
      	
      	const prevCount = cardsInPlay.length;
		cardsInPlay = cardsInPlay.filter(card=>!isWinningCard(card));
      	const newCount = cardsInPlay.length;
      	const delta = prevCount - newCount;
      	str += ` - WINNERS: ${delta} - CARDS LEFT: ${newCount}`;
	}
	console.log(str);
  	index++;
}
